...
- **🛡️ Guardrailed naming** — one flat 2–4 word name per group, never a copied tab title, never a bare site name, never a vague "General / Misc" bucket, never a one-tab group: the prompt carries anti-examples for each of these real failure modes, and the parser + engine enforce them deterministically (stray `Topic / Detail` answers merge into one group; stale singletons are dissolved).
