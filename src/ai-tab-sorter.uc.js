...
(() => {
  ...
  class ButtonInjector {
    ...
    static anchors() {
      const list = [
        ...document.querySelectorAll(".pinned-tabs-container-separator"),
      ];
    ...
    static ensure() {
      ...
      if (!anchors.length) log("no tab-strip anchor found for buttons (yet)");
    }
    ...
    static reattachHooks() {
      ...
      try {
        ...
        if (ws && !ws.__aiTabSorterHooked) {
          ...
          for (const name of ["onTabBrowserInserted", "updateTabsContainers"]) {
            const orig = ws[name];
  ...
  class Bootstrap {
    ...
    static async init() {
      ...
      if (PrefStore.get("enabled") && PrefStore.get("showButtons")) {
        ...
        setTimeout(() => ButtonInjector.ensure(), 1000); // catch late strips
        setTimeout(() => ButtonInjector.ensure(), 3000);
      }
